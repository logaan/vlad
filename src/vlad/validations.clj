...
(defn equals-value
  ...
  (predicate selector #(not (= value %))
             {:type ::equals-value :value value :selector selector}))
...
(defn matches
  ...
  (predicate selector #(nil? (re-matches pattern %))
             {:type ::matches :pattern pattern :selector selector}))
